...
const form = ref({ users: 5, threads: 10, posts: 30, categories: 3 });
...
async function importData() {
    ...
    try {
        const { data } = await adminApi.importDummyData({
            ...
            categories: Number(form.value.categories),
        });
        const c = data.data;
        success.value = `Generated ${c.categories} categories, ${c.users} users, ${c.threads} threads and ${c.posts} posts.`;
        await loadStatus();
    } catch (err) {
...
async function deleteData() {
    ...
    try {
        ...
        const d = data.data;
        success.value = `Deleted ${d.users} users, ${d.threads} threads, ${d.posts} posts and ${d.categories} categories.`;
        await loadStatus();
    } catch (err) {
        ...
        <template v-else>
            ...
            <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div class="card rounded-2xl p-4 text-center">
                    <p class="text-2xl font-bold text-ink-900">{{ status?.categories ?? 0 }}</p>
                    <p class="text-xs font-medium text-ink-500">Dummy categories</p>
                </div>
                <div class="card rounded-2xl p-4 text-center">
            ...
            <div class="card mt-4 rounded-2xl p-5">
                ...
                <p class="mt-1 text-sm text-ink-500">
                    Creates categories (with emoji icons) and fake members (with a <span class="font-mono text-xs">@dummy.forum</span> email)
                    who write threads and replies across them, with votes and randomized timestamps so lists look lived-in.
                    Works on a completely empty forum.
                </p>

                <form class="mt-4 flex flex-wrap items-end gap-3" @submit.prevent="importData">
                    <div class="w-28">
                        <label class="field-label" for="dd-categories">Categories</label>
                        <input id="dd-categories" v-model="form.categories" type="number" min="0" max="20" required class="input" />
                    </div>
                    <div class="w-28">
            ...
            <div class="card mt-4 rounded-2xl border-rose-200 p-5">
                ...
                <p class="mt-1 text-sm text-ink-500">
                    Removes every dummy user along with all their threads, posts and votes, plus generated
                    categories that ended up empty. Content created by real members is never touched — a
                    generated category with real threads in it stays.
                </p>
                <button
                    type="button"
                    ...
                    :disabled="deleting || (!status?.users && !status?.categories)"
                    @click="deleteData"
                >
                ...
                <p v-if="!status?.users && !status?.categories" class="mt-2 text-xs text-ink-400">Nothing to delete right now.</p>
            </div>
        </template>
    </div>
</template>
